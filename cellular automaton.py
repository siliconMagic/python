...
def make_rules(list):
    rules = []
    rules.append(['...', list[0]])
    rules.append(['..x', list[1]])
    rules.append(['.x.', list[2]])
    rules.append(['.xx', list[3]])
    rules.append(['x..', list[4]])
    rules.append(['x.x', list[5]])
    rules.append(['xx.', list[6]])
    rules.append(['xxx', list[7]])
#    print rules # as a list
    rules = dict(rules)
#    print rules # as a dict
    return rules
    
def make_next_gen(s, rules):
    next_s = ''
    for i in range(len(s)):
#        print i, len(s)
        if i+1 == len(s):
            pattern = s[i-1] + s[i] + s[0]
        else:
            pattern = s[i-1] + s[i] + s[i+1]
        next_s = next_s + rules[pattern]
    return next_s
        

def cellular_automaton(string, pattern, generations):
    n = 1
    table = make_rules(make_pattern(pattern))
#    print string
    while n <= generations:
