...
# for a continuous return number
daily_return = np.log(1 + daily_price.pct_change())
# cumulative_return = daily_return.cumsum()        useful for a normalized
# return chart

# <codecell>

# create expected return vector, stdev vector and covariance, correlation
# matrices
...
print("Equal weighted return: {:.2%} with a {:.2%} volatility".format(
    Equal_return, Equal_vol.item()))

# <codecell>

# C is the covariance matrix as an NxN matrix of real numbers


def make_LMmat(C, R, Muvec, Onevec):
...
def MinVar_target_return(C, LMmat, LMtarget):
    ...
    # SD is standard deviation - measures volatility
...
print(
    "For a return of {:.2%} the minimum volatility is: {:.2%}".format(r, target_SD.item()))
...
for i, weight in enumerate(Xminvec):
    print(i, weight)
print("Min Variance return: {:.2%} with a {:.2%} volatility".format(
    Xmin_return.item(), Xmin_SD.item()))
...
def Sharpe_port(C, Muvec, rf):
    ...
    XSh_SD = np.sqrt(1 / np.sum(C.I * Muhat)
                     ** 2 * Muhat.T * C.I * C * C.I * Muhat)
...
for i, weight in enumerate(XShvec):
    print(i, weight)
print("Sharpe Optimal return: {:.2%} with volatility: {:.2%}".format(
    XSh_return.item(), XSh_SD.item()))
...
def Find_Frontier(C, Muvec, rf):
    ...
    frontier_weights = []
    r_points = np.arange(
        min(Muvec) / 1.2, max(Muvec) * 1.2, (max(Muvec) - min(Muvec)) / 100)
    for r in r_points:
...
# plt.plot([Xmin_return.item(), Xmin_return.item()],
#          [0, 1], color='orange', lw=2)
